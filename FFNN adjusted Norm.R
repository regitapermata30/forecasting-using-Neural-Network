...
#menggunakan preprocessing adjusted normalized
...
#preprocessing using adjusted normalized
min.Yt = min(y1t)
max.Yt   = max(y1t)
Yt_std <- (2*(y1t-min.Yt)/(max.Yt-min.Yt))-1 #nilai -1 s.d 1
...
k1=0;mm=0;for (k in seq_along(neuron)){
  for (a1 in seq(aa)){
    set.seed(1234)
    mm <- mm+1
    best.model_NN=neuralnet(formula=form,data=data,hidden=neuron[k], algorithm = "backprop",
    ...
    for (repl in 1:rep){
      fits.model=((as.ts(unlist(best.model_NN$net.result[[repl]])))+1)*((max.Yt-min.Yt)+2*min.Yt)/2
    ...
    fore.model_NN[,mm] <- (Ytest[(nrow(data)+1):(nrow(data)+n_fore)]+1)*((max.Yt-min.Yt)+2*min.Yt)/2
    k1 <- k1+rep
  }
